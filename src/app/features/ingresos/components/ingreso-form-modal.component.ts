...
export class IngresoFormModalComponent {
    ...
    searchClientees(event: AutoCompleteCompleteEvent) {
        const query = event.query;
        if (!query || query.length < 2) {
            this.clienteStore.getRecent(5).then((data) => this.filteredClientees.set(data)).catch(() => this.filteredClientees.set([]));
        } else {
            this.clienteStore.search(query, 10).then((data) => this.filteredClientees.set(data)).catch(() => this.filteredClientees.set([]));
        }
    }
    ...
    onClienteClear() { this.selectedCliente = null; this.formData.clienteId = undefined; this.formData.clienteNombre = undefined; this.filteredClientees.set([]); }
