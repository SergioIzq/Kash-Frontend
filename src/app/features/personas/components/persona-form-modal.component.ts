...
@Component({
    ...
    template: `
        <p-dialog 
            [(visible)]="isVisible" 
            [style]="{ width: '450px' }" 
            [header]="isEditMode() ? 'Editar Persona' : 'Nueva Persona'" 
            [modal]="true" 
            ...
            <ng-template #content>
                <div class="flex flex-col gap-4">
                    <div>
                        <label for="nombre" class="block font-bold mb-3">Nombre de la Persona *</label>
                        <input 
                            type="text" 
                            pInputText 
                            id="nombre" 
...
export class PersonaFormModalComponent {
    private messageService = inject(MessageService);

    // Inputs/Outputs
    visible = input<boolean>(false);
    persona = input<Partial<Persona> | null>(null);
    visibleChange = output<boolean>();
    save = output<Partial<Persona>>();
    cancel = output<void>();
    ...
    constructor() {
        ...
        // Cargar datos cuando cambia la persona
        effect(() => {
