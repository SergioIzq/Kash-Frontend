...
6. **Implement tasks (loop until done or blocked)**
   ...
   - Before touching code that depends on existing behavior (a function signature, a component prop, a config key, a return shape), read it first - don't assume it matches conventions from other codebases
...
- Don't invent - never rely on an API, library, config key, or file path unless you've verified it exists in this codebase this session; if something needed doesn't exist yet, say so and ask instead of assuming it does
- Ground every implementation choice in what you've actually read here, not in generic patterns typical of "how these things are usually done"
